...
class FinTextModel(nn.Module):
    def __init__(self, config=None):
        ...
        self.ko_model = ElectraModel.from_pretrained(
            "monologg/koelectra-base-v3-discriminator"
        )
        self.kc_tokenizer = ElectraTokenizer.from_pretrained(
    ...
    def forward(self, x):
        ...
        x_dict["price_vector"] = []
        for period in x:
            article_matrix = self.embed_text(
                period["ArticleText"], self.ko_tokenizer, self.ko_model
            )
            community_matrix = self.embed_text(
